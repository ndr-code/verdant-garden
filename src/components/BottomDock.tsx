...
export const BottomDock = ({ onClockClick, onTimerClick }: BottomDockProps) => {
  return (
    <TooltipProvider>
      <div className='h-16 bg-white/10 w-full flex-shrink-0 border-t-2 border-white/20 flex items-center justify-center gap-8 px-8'>
        ...
        <Tooltip content='Notes'>
          <StickyNote className='w-8 h-8 text-white cursor-pointer hover:text-amber-300 transition-colors' />
        </Tooltip>

        <Tooltip content='Goals'>
          <Target className='w-8 h-8 text-white cursor-pointer hover:text-amber-300 transition-colors' />
        </Tooltip>

        <Tooltip content='Music'>
          <Music className='w-8 h-8 text-white cursor-pointer hover:text-amber-300 transition-colors' />
        </Tooltip>
      </div>
    </TooltipProvider>
  );
